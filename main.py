...
# Sửa đường dẫn file save cho Android
if sys.platform == 'android':
    try:
        from android.storage import app_storage_path
        SAVE_DIR = app_storage_path()
        GAME_SAVE_FILE = os.path.join(SAVE_DIR, "fishing_save.json")
    except:
...
# Thêm try-except cho các hàm có thể lỗi trên Android
def android_safe_quit():
    try:
        pygame.quit()
        if sys.platform == 'android':
            import android
...
# --- CẤU HÌNH KÍCH THƯỚC MÀN HÌNH TỐI ƯU ---
TARGET_WIDTH = 1080
TARGET_HEIGHT = 2280

# Khởi tạo pygame với xử lý lỗi cho Android
try:
    pygame.init()
    
    # Kiểm tra nền tảng để cấu hình màn hình phù hợp
    if sys.platform == 'android':
        # Trên Android, sử dụng chế độ toàn màn hình
        info = pygame.display.Info()
        SCREEN_WIDTH = info.current_w
        SCREEN_HEIGHT = info.current_h
        screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.FULLSCREEN)
    else:
        # Trên desktop, sử dụng logic hiện tại
        info = pygame.display.Info()
        device_w = info.current_w
        device_h = info.current_h

        if device_w >= TARGET_WIDTH and device_h >= TARGET_HEIGHT:
            SCREEN_WIDTH = TARGET_WIDTH
            SCREEN_HEIGHT = TARGET_HEIGHT
            screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
        else:
            SCREEN_WIDTH = device_w if device_w != 0 else 800
            SCREEN_HEIGHT = device_h if device_h != 0 else 600
            screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT), pygame.FULLSCREEN) 

    pygame.display.set_caption("Cắm Câu Cá Lóc")
    clock = pygame.time.Clock()

except Exception as e:
    print(f"Lỗi khởi tạo pygame: {e}")
    # Fallback cấu hình cơ bản nếu có lỗi
    SCREEN_WIDTH = 800
    SCREEN_HEIGHT = 600
    screen = pygame.display.set_mode((SCREEN_WIDTH, SCREEN_HEIGHT))
    clock = pygame.time.Clock()
...
# Thử load ảnh nền với xử lý lỗi cho Android
try:
    # Thay "background.jpg" bằng tên file ảnh của bạn
    # Các định dạng hỗ trợ: JPG, PNG, BMP, etc.
    background_image = pygame.image.load("background.jpg")
    background_image = pygame.transform.scale(background_image, (SCREEN_WIDTH, SCREEN_HEIGHT))
    background_rect = background_image.get_rect()
    print("Đã load ảnh nền thành công!")
except Exception as e:
    print(f"Không thể load ảnh nền: {e}")
...
# --- [MỚI: SỰ KIỆN] ---
EVENT_DURATION = 5 * 60  # 5 phút (giây)
EVENT_COOLDOWN = 5 * 60  # 5 phút (giây) - Thời gian chờ sau khi kết thúc sự kiện
current_event = None
event_start_time = 0
event_end_time = 0
event_cooldown_end = 0
last_event_check_time = time.time()  # Thời gian kiểm tra sự kiện lần cuối

# --- [MỚI: SỰ KIỆN GIÁNG SINH] ---
CHRISTMAS_EVENT_DURATION = 7 * 24 * 60 * 60  # 7 ngày (giây)
CHRISTMAS_EVENT_START = None  # Sẽ được thiết lập khi sự kiện bắt đầu
CHRISTMAS_EVENT_END = None
is_christmas_event_active = False

# Thiết lập bố cục màn hình
...
COIN_COLOR = (255, 215, 0) # Vàng kim cho Xu
MARKET_COLOR = (180, 180, 0) # Màu vàng nâu cho Chợ
EVENT_COLOR = (255, 105, 180) # Màu hồng cho sự kiện
CHRISTMAS_COLOR = (255, 0, 0) # Màu đỏ Giáng Sinh

FONT_NAME = 'Arial' 
try:
    FONT_VERY_SMALL = pygame.font.SysFont(FONT_NAME, int(SCREEN_HEIGHT * 0.012))
    FONT_SMALL = pygame.font.SysFont(FONT_NAME, int(SCREEN_HEIGHT * 0.018)) 
    FONT_MEDIUM = pygame.font.SysFont(FONT_NAME, int(SCREEN_HEIGHT * 0.024)) 
    FONT_LARGE = pygame.font.SysFont(FONT_NAME, int(SCREEN_HEIGHT * 0.035)) 
except:
    # Fallback font cho Android
...
# --- HẰNG SỐ CẦN CÂU VÀ KINH TẾ (ĐÃ SỬA) ---
MAX_LEVEL_ROD = 10 
...
ROD_TEMPLATES = {
    ...
    # --- [MỚI: CẦN CÂU GIÁNG SINH] ---
    "Cần Giáng Sinh": {
        "base_rate_bonus": 0.015, 
        "base_exp_bonus": 0.3, 
        "max_level": 1,  # Không thể nâng cấp
        "rate_per_level": 0.0, 
        "exp_per_level": 0.0,
        "base_cast_time": 90, 
        "base_upgrade_cost": 0, 
        "buy_price": None,
        "expiry_time": None  # Thời gian hết hạn
    }
}
DEFAULT_ROD_TEMPLATE = "Cần Tre"

# Giá bán cá cho thương lái
FISH_PRICE_PER_KG = {
    "Cá Lóc": 100, 
}

# --- [MỚI: VẬT PHẨM GIÁNG SINH] ---
CHRISTMAS_ITEMS = {
...
CHRISTMAS_EXCHANGE_REQUIREMENT = {
    ...
    "Mũ Giáng Sinh": 10, 
    "Kẹo Giáng Sinh": 10,
    "Cá Lóc": 10.0  # 10kg cá lóc
}

CHRISTMAS_EXCHANGE_SUCCESS_RATE = 0.05  # 5% tỉ lệ thành công

# --- Dữ liệu Vị trí & Kênh ---
LOCATIONS = {
    "Khu Đồng Lớn": {
        "Rate_Per_Minute": 0.15,  # ĐÃ THAY ĐỔI: từ 0.005 lên 0.15 (15%)
        "required_level": 1, 
        "Channels": {
            "Kênh A": {},
            "Kênh B": {},
    ...
    "Khu Rừng Tràm": {
        "Rate_Per_Minute": 0.10,  # ĐÃ THAY ĐỔI: từ 0.010 lên 0.10 (10%)
        "required_level": 10, 
        "Channels": {
            "Kênh C": {},
            "Kênh D": {},
...
# --- Dữ liệu Game (Khởi tạo Biến toàn cục SỚM) ---
player_level = 1 
player_exp = 0
player_coins = 0 
...
rods_inventory = [] 
fish_inventory = [] # Lưu dưới dạng list of dict: [{'name': 'Cá Lóc', 'weight': 1.2, 'exp': 50}]
# --- [MỚI: VẬT PHẨM GIÁNG SINH] ---
christmas_items_inventory = [] # Lưu vật phẩm Giáng Sinh: [{'name': 'Tất Giáng Sinh', 'weight': 0.1}, ...]
...
# Biến tạm cho màn hình bán cá
fish_to_sell_selected_type = None 
fish_to_sell_quantity = 0 

# --- [MỚI: MARKET] ---
market_slot_end_time = 0 
market_selling_fish = [] # Cá đang được bày bán: list of dict {'id': fish_id, 'name': name, 'weight': weight}
market_customer = None # {'name': 'Khách A', 'offer': 1500, 'fish_id': fish_id, 'time': time.time()}
last_customer_time = time.time()
CUSTOMER_SPAWN_INTERVAL = 10 # Khách xuất hiện sau 10 giây (nếu không có ai)
...
def end_christmas_event():
    ...
    # Vô hiệu hóa cần câu Giáng Sinh đã hết hạn
    now = time.time()
...
def get_random_christmas_item():
    """Trả về một vật phẩm Giáng Sinh ngẫu nhiên."""
...
def add_christmas_item_to_inventory(item_name, weight):
    """Thêm vật phẩm Giáng Sinh vào kho đồ."""
...
def get_christmas_items_summary():
    """Tóm tắt số lượng vật phẩm Giáng Sinh đang có."""
    summary = {}
...
def can_exchange_christmas_gift():
    """Kiểm tra xem có đủ điều kiện để đổi quà Giáng Sinh không."""
    summary = get_christmas_items_summary()
    fish_summary = get_fish_inventory_summary()
    
    # Kiểm tra vật phẩm Giáng Sinh
...
def exchange_christmas_gift():
    """Thực hiện việc đổi quà Giáng Sinh."""
    ...
    for item_name, required in CHRISTMAS_EXCHANGE_REQUIREMENT.items():
        if item_name == "Cá Lóc":
            ...
            # Giữ lại các vật phẩm không cần xóa
            kept_items = []
            removed_count = 0
    ...
    # Kiểm tra tỉ lệ thành công 5%
    if random.random() < CHRISTMAS_EXCHANGE_SUCCESS_RATE:
        # Thành công: Nhận cần câu Giáng Sinh
        expiry_time = time.time() + (5 * 24 * 60 * 60)  # 5 ngày
        ...
        # Thất bại: Nhận Xu ngẫu nhiên
        # Tỉ lệ: 5k (40%), 10k (25%), 20k (20%), 30k (10%), 50k (5%)
...
def check_event_status():
    ...
    # Kiểm tra tạo sự kiện mới mỗi phút với xác suất
    ...
    # --- [MỚI: KIỂM TRA SỰ KIỆN GIÁNG SINH] ---
    check_christmas_event_status()

def start_random_event():
    """Bắt đầu một sự kiện ngẫu nhiên với xác suất mới."""
    global current_event, event_start_time, event_end_time
    
    # Tạo số ngẫu nhiên để chọn sự kiện
    rand_val = random.random()
    
    # Tất cả sự kiện có xác suất 20% như nhau
    if rand_val < 0.20:  # 20% cho Cá Lóc Hoàng Kim
        current_event = {
    ...
    elif rand_val < 0.40:  # 20% cho Ngày Hội EXP
        current_event = {
            "name": "Ngày Hội EXP", 
            "description": "Nhận thêm 50% EXP khi bắt cá!",
            "effect": "boost_exp",
            "color": (0, 255, 150)  # Xanh lá
        }
    elif rand_val < 0.60:  # 20% cho Ngày Sốt Giá
        current_event = {
            "name": "Ngày Sốt Giá",
            "description": "NPC trong chợ mua với giá cao hơn 50%!",
            "effect": "boost_market",
            "color": (200, 200, 200)  # Bạc
        }
    else:
        # Không có sự kiện (40%)
        return
...
def get_upgrade_cost(rod):
    ...
    # Chi phí cấp tiếp theo (L+1) = Base_Cost * (Multiplier ^ L)
    cost = base_cost * (UPGRADE_COST_MULTIPLIER ** (current_level - 1)) # -1 vì level bắt đầu từ 1
...
def check_fish(rate_per_minute, rod_rate_bonus=0, rod_exp_bonus=0):
    """Tính toán tên cá, trọng lượng và EXP khi dính cá (mặc định là Cá Lóc)."""
    # Áp dụng hệ số sự kiện
...
def upgrade_rod(rod_id):
    """Thực hiện nâng cấp cần câu bằng Xu."""
    global player_coins
...
def get_fish_inventory_summary():
    """Tóm tắt số lượng và trọng lượng cá đã bắt, thêm ID để bán tại chợ."""
    summary = {}
    for i, fish in enumerate(fish_inventory):
        fish_id = fish.get('id', i) # Dùng index tạm thời hoặc ID nếu đã có
        fish['id'] = fish_id # Đảm bảo cá có ID
        name = fish['name']
        if name not in summary:
...
def sell_fish(fish_name, quantity):
    """Bán cá cho Thương Lái theo số lượng và tính tiền theo trọng lượng."""
    ...
    # Xóa cá đã bán khỏi kho đồ
    new_fish_inventory = []
    sold_count = 0
    
    # Sử dụng ID (hoặc index) để tránh trùng lặp
    fish_to_sell_ids = [f.get('id', idx) for idx, f in enumerate(fish_to_sell)]
    
    # Tạo lại fish_inventory, chỉ giữ lại những con cá chưa được bán
    
    for idx, fish in enumerate(fish_inventory):
        fish_id = fish.get('id', idx)
        
        # Chỉ giữ lại cá nếu nó không nằm trong danh sách cá cần bán
...
def generate_fish_id():
    """Tạo ID duy nhất cho cá."""
    # Dùng timestamp/random để đảm bảo ID không trùng lặp
    return int(time.time() * 1000) + random.randint(1, 1000)

def prepare_fish_inventory():
    """Đảm bảo mọi con cá đều có ID."""
    for i, fish in enumerate(fish_inventory):
        if fish.get('id') is None:
            fish['id'] = generate_fish_id()

# --- [MỚI: MARKET] - LOGIC KHÁCH HÀNG ---
def check_for_customer():
    """Kiểm tra và tạo khách hàng ngẫu nhiên."""
    global market_customer, last_customer_time, market_selling_fish
    
    if market_customer: return # Đã có khách
    if not market_selling_fish: return # Không có cá để bán

    now = time.time()
    if now - last_customer_time < CUSTOMER_SPAWN_INTERVAL: return # Giới hạn tần suất khách
    
    # Xác suất xuất hiện khách (ví dụ: 20% mỗi lần kiểm tra)
    if random.random() < 0.2: 
        
        # Chọn ngẫu nhiên 1 con cá đang bày bán
        fish_to_buy = random.choice(market_selling_fish)
        
        price_per_kg = FISH_PRICE_PER_KG.get(fish_to_buy['name'], 0)
        
        # Tính toán giá trị Thương Lái
        merchant_value = fish_to_buy['weight'] * price_per_kg
        
        # Áp dụng hệ số sự kiện cho giá NPC
        market_multiplier = get_event_multiplier("market")
        
        # Tạo giá ngẫu nhiên (từ 70% đến 150% giá Thương Lái, có thể cao hơn nếu có sự kiện)
        # Tỷ lệ: 30% giá thấp, 40% giá bằng/hợp lý, 30% giá cao
        rand_val = random.random()
        
        max_multiplier = 1.5 * market_multiplier  # Tối đa 150% (có thể lên đến 225% với sự kiện)
        
        if rand_val < 0.3: # Giá thấp (70% - 99% giá Thương Lái)
            offer_multiplier = random.uniform(0.70, 0.99)
            customer_name = random.choice(["Lái Buôn Keo Kiệt", "Bà Tám Kì Kèo"])
        elif rand_val < 0.7: # Giá hợp lý (100% - 110% giá Thương Lái)
            offer_multiplier = random.uniform(1.00, 1.10)
            customer_name = random.choice(["Ông Tư Hào Phóng", "Cô Ba Dễ Tính"])
        else: # Giá cao (111% - max_multiplier giá Thương Lái)
        ...
        market_customer = {
            'name': customer_name,
            'offer': max(1, offer_price), # Đảm bảo giá không dưới 1 xu
...
def accept_market_offer():
    """Đồng ý bán cá cho khách hàng tại Chợ."""
    ...
    # 2. Xóa cá khỏi danh sách bày bán
    market_selling_fish = [f for f in market_selling_fish if f['id'] != fish_id]
    
    # 3. Xóa cá khỏi kho đồ chính (dựa vào ID)
...
def reject_market_offer():
    """Từ chối bán cá cho khách hàng tại Chợ."""
    ...
    market_customer = None
    last_customer_time = time.time() - CUSTOMER_SPAWN_INTERVAL + 5 # Sớm có khách mới
    
    return True, msg

def reset_market_state():
    """Reset trạng thái chợ khi rời khỏi."""
    global market_slot_end_time, market_selling_fish, market_customer
    market_slot_end_time = 0
    
    # Nếu đang có khách trả giá, hủy giao dịch
    if market_customer:
        display_message("Hủy giao dịch chợ! Khách hàng bỏ đi.", 3)
        market_customer = None
        
    # Cá đang bày bán trở về kho (vì nó vẫn luôn ở kho, chỉ là bị đánh dấu)
    market_selling_fish = [] 
    

# --- HÀM MỚI: CHUẨN BỊ DỮ LIỆU THĂM TRỘM ---
def prepare_steal_slots():
    """Chuẩn bị 3 slot thăm trộm với xác suất dính cá ngẫu nhiên."""
...
# --- Lưu/Tải Game (ĐÃ CẬP NHẬT MARKET & SỰ KIỆN & GIÁNG SINH) ---
...
def load_game():
    ...
    # --- [CẬP NHẬT] Tăng số cần ban đầu lên 5 ---
    INITIAL_RODS = 5 
    ...
    try:
        ...
        # --- [MỚI: SỰ KIỆN GIÁNG SINH] - Xử lý sự kiện Giáng Sinh khi offline ---
        ...
        if not load_data.get("rods_inventory"):
             # --- [CẬP NHẬT] Khởi tạo 5 cần nếu không có save file inventory ---
                            ...
                            if random.random() < adjusted_rate: 
                                ...
                                if weight > 0:
                                    ...
                                    fish_inventory.append({
                                        ...
                                        "chan": rod["channel"], 
                                        "exp": exp_gained,
                                        "id": generate_fish_id() # Gán ID khi offline
                                    })
                                    
                                    # --- [MỚI: VẬT PHẨM GIÁNG SINH OFFLINE] ---
                                    if christmas_item:
        ...
        # --- [CẬP NHẬT] Khởi tạo 5 cần nếu không có save file ---
        rods_inventory = [create_default_rod(i, "Cần Tre") for i in range(INITIAL_RODS)]
        player_coins = 1000 
        ...
        # --- [CẬP NHẬT] Khởi tạo 5 cần nếu lỗi tải file ---
        rods_inventory = [create_default_rod(i, "Cần Tre") for i in range(INITIAL_RODS)] 
        player_coins = 1000
        MAX_RODS = INITIAL_RODS
        christmas_items_inventory = []
        
# --- GỌI HÀM TẢI GAME SAU KHI KHỞI TẠO BIẾN TOÀN CỤC ---
load_game() 


# --- HÀM ONLINE CHECK & HP REGEN & SỰ KIỆN ---
...
def update_online_status():
                    ...
                    if random.random() < adjusted_rate: 
                        ...
                        if weight > 0:
                            ...
                            rod["catch_data"] = {
                                ...
                                "loc": rod["location"], 
                                "chan": rod["channel"],
                                "id": generate_fish_id(), # Gán ID khi dính cá
                                "christmas_item": christmas_item  # --- [MỚI] ---
                            }
                            break 
    ...
    # --- [MỚI: MARKET] - 3. Kiểm tra hết giờ chợ và Khách hàng ---
    if game_state == "MARKET_SELL":
        if market_slot_end_time > 0 and now >= market_slot_end_time:
            ...
            check_for_customer() # Kiểm tra khách hàng nếu đang bán
    
    # --- [MỚI: SỰ KIỆN] - 4. Kiểm tra sự kiện ---
    check_event_status()

# --- HÀM VẼ (Tối ưu hóa và Tách màn hình) ---
...
def draw_character_screen():
    ...
    # --- [MỚI: HIỂN THỊ VẬT PHẨM GIÁNG SINH] ---
...
def draw_shop_screen():
    ...
    # --- [SỬA: NÂNG CẤP GIỚI HẠN CẦN CÂU VỚI CHI PHÍ CẤP SỐ NHÂN] ---
    upgrade_rod_limit_cost = 50000 * (2 ** (MAX_RODS - 5))  # Cấp số nhân: 50k, 100k, 200k, 400k, 800k
    y_start_upgrade = y_start + y_step * 2
    
    upgrade_can_buy = player_coins >= upgrade_rod_limit_cost and MAX_RODS < 6  # Tăng giới hạn lên 6
    ...
    if MAX_RODS < 6:  # Tối đa 6 cần câu
...
# --- [MỚI: MARKET] - VẼ MÀN HÌNH CHỢ (MARKET MENU) ---
...
# --- [MỚI: MARKET] - VẼ MÀN HÌNH BÁN CÁ (MARKET SELL) ---
def draw_market_sell_screen():
    ...
    for i, fish in enumerate(market_selling_fish):
        ...
        # Nút đang bày bán, không cần click (trừ khi rút về)
    ...
    # 3. Nút Quay Lại/Rút lui
    
    # Rút lui (chỉ được rút khi không có khách)
...
# --- [MỚI: MARKET] - VẼ MÀN HÌNH CHỌN CÁ BÁN TẠI CHỢ ---
def draw_market_select_fish():
    ...
    # Đảm bảo cá có ID trước khi hiển thị
    prepare_fish_inventory() 
    
    LOCATIONS["Market_Fish_Inventory"] = []
    
    # --- [SỬA: CHỈ HIỂN THỊ CÁ TRÊN 1KG] ---
    selling_fish_ids = [f['id'] for f in market_selling_fish]
    fish_available_to_sell = [f for f in fish_inventory if f.get('id') not in selling_fish_ids and f['weight'] >= 1.0]
    
    if not fish_available_to_sell:
        draw_text(screen, "Không còn cá trên 1kg trong kho để bày bán.", FONT_MEDIUM, RED_FISH, SCREEN_WIDTH // 2, y_start_list + 50, center=True)
...
# --- [MỚI: SỰ KIỆN GIÁNG SINH] - VẼ MÀN HÌNH QUÀ GIÁNG SINH ---
def draw_christmas_gift_screen():
    ...
    # Hiển thị vật phẩm Giáng Sinh đang có
    y_start = int(SCREEN_HEIGHT * 0.15)
    ...
    # Hiển thị thông báo nếu đã chọn đổi
...
def draw_main_menu():
    global player_level, player_exp, player_health, STEAL_COST, player_coins, current_event, event_end_time, is_christmas_event_active, CHRISTMAS_EVENT_END
    
    # --- [MỚI: VẼ ẢNH NỀN] ---
    if background_image:
        screen.blit(background_image, background_rect)
    else:
        screen.fill(MENU_COLOR)  # Fallback nếu không có ảnh nền
    
    # --- Tạo lớp phủ trong suốt để chữ dễ đọc ---
    overlay = pygame.Surface((SCREEN_WIDTH, SCREEN_HEIGHT))
    overlay.set_alpha(128)  # Độ trong suốt (0-255, 128 = 50% trong suốt)
    overlay.fill(BLACK)
    ...
    # --- [MỚI: SỰ KIỆN GIÁNG SINH] - Hiển thị thông tin sự kiện Giáng Sinh ---
    y_offset = TITLE_Y + int(SCREEN_HEIGHT * 0.1)
    
    if is_christmas_event_active:
        now = time.time()
        time_left = max(0, CHRISTMAS_EVENT_END - now)
        ...
        event_text = f"SỰ KIỆN GIÁNG SINH - Còn {days_left} ngày {hours_left} giờ"
        draw_button(screen, event_rect, event_text, FONT_MEDIUM, CHRISTMAS_COLOR, WHITE)
        
        # Hiển thị mô tả sự kiện
        desc_y = event_rect.bottom + 5
        draw_text(screen, "Câu dính cá Lóc có 50% cơ hội nhận vật phẩm Giáng Sinh!", FONT_SMALL, WHITE, 
                 SCREEN_WIDTH // 2, desc_y, center=True)
        
        y_offset = desc_y + int(SCREEN_HEIGHT * 0.05)
    
    # --- [MỚI: SỰ KIỆN THƯỜNG] - Hiển thị thông tin sự kiện thường ---
    if current_event:
        now = time.time()
        time_left = max(0, event_end_time - now)
        minutes_left = int(time_left // 60)
        seconds_left = int(time_left % 60)
        
        event_rect = pygame.Rect(BUTTON_MARGIN, y_offset, 
                                SCREEN_WIDTH - 2 * BUTTON_MARGIN, BUTTON_H)
        event_color = current_event.get("color", EVENT_COLOR)
        
        event_text = f"{current_event['name']} - {minutes_left}:{seconds_left:02d}"
        draw_button(screen, event_rect, event_text, FONT_MEDIUM, event_color, WHITE)
        
        # Hiển thị mô tả sự kiện nhỏ hơn bên dưới
        desc_y = event_rect.bottom + 5
        draw_text(screen, current_event['description'], FONT_SMALL, WHITE, 
                 SCREEN_WIDTH // 2, desc_y, center=True)
        
        y_start = desc_y + int(SCREEN_HEIGHT * 0.05)
    else:
        y_start = y_offset + int(SCREEN_HEIGHT * 0.05)
              
    BUTTON_W = SCREEN_WIDTH - 2 * BUTTON_MARGIN
    
    # HÀNG 1
    char_rect = pygame.Rect(BUTTON_MARGIN, y_start, BUTTON_W, BUTTON_H)
    LOCATIONS["Char_Info_Rect"] = draw_button(screen, char_rect, "THÔNG TIN NHÂN VẬT", FONT_MEDIUM, BLUE_WATER, WHITE)
    
    # HÀNG 2
    inventory_rect = pygame.Rect(BUTTON_MARGIN, y_start + BUTTON_Y_STEP, BUTTON_W, BUTTON_H)
    LOCATIONS["Inventory_Rect"] = draw_button(screen, inventory_rect, "TÚI ĐỒ & SHOP", FONT_MEDIUM, YELLOW_BUTTON, BLACK)
    
    # HÀNG 3 - [MỚI: MARKET]
    market_rect = pygame.Rect(BUTTON_MARGIN, y_start + 2 * BUTTON_Y_STEP, BUTTON_W, BUTTON_H)
    LOCATIONS["Market_Rect"] = draw_button(screen, market_rect, "CHỢ CÁ TỰ DO", FONT_MEDIUM, MARKET_COLOR, WHITE)
    
    # HÀNG 4
    start_rect = pygame.Rect(BUTTON_MARGIN, y_start + 3 * BUTTON_Y_STEP, BUTTON_W, BUTTON_H)
    LOCATIONS["Start_Rect"] = draw_button(screen, start_rect, "ĐI CẮM CÂU", FONT_MEDIUM, GREEN_BG, WHITE)
    
    # HÀNG 5
    steal_rect = pygame.Rect(BUTTON_MARGIN, y_start + 4 * BUTTON_Y_STEP, BUTTON_W, BUTTON_H)
    ...
    # --- [MỚI: NÚT SỰ KIỆN GIÁNG SINH] ---
    if is_christmas_event_active:
        christmas_rect = pygame.Rect(BUTTON_MARGIN, y_start + 5 * BUTTON_Y_STEP, BUTTON_W, BUTTON_H)
        LOCATIONS["Christmas_Rect"] = draw_button(screen, christmas_rect, "QUÀ GIÁNG SINH", FONT_MEDIUM, CHRISTMAS_COLOR, WHITE)
        y_quit = y_start + 6 * BUTTON_Y_STEP
    else:
        y_quit = y_start + 5 * BUTTON_Y_STEP
...
def handle_click(pos):
    ...
    if game_state == "MAIN_MENU":
        ...
        if "Steal_Rect" in LOCATIONS and LOCATIONS["Steal_Rect"].collidepoint(pos):
            ...
            return
        # --- [MỚI: SỰ KIỆN GIÁNG SINH] ---
        if "Christmas_Rect" in LOCATIONS and LOCATIONS["Christmas_Rect"].collidepoint(pos) and is_christmas_event_active:
            christmas_exchange_selected = False
            game_state = "CHRISTMAS_GIFT"
            return
        # --- [MỚI: SỰ KIỆN] - Đã bỏ nút kích hoạt sự kiện vì sự kiện tự động ---
    ...
    # --- [MỚI: MARKET] - Xử lý click Market Menu ---
        ...
        if "Market_Go_Selling_Rect" in LOCATIONS and LOCATIONS["Market_Go_Selling_Rect"] and LOCATIONS["Market_Go_Selling_Rect"].collidepoint(pos):
            if market_slot_end_time > time.time():
                 # Nếu đã có cá bày bán thì vào luôn, không thì vào màn hình chọn cá
    ...
    # --- [MỚI: MARKET] - Xử lý click Chọn Cá Bán tại Chợ ---
        ...
        if "Back_Rect_Market_Select_Fish" in LOCATIONS and LOCATIONS["Back_Rect_Market_Select_Fish"].collidepoint(pos):
            # Hủy quá trình chọn cá, giữ lại xu đã thuê
            market_selling_fish = [] 
            game_state = "MARKET_MENU"
    ...
    # --- [MỚI: MARKET] - Xử lý click Bán Cá tại Chợ ---
        ...
        # Rút lui (chỉ khi không có khách)
        if not market_customer and "Back_Rect_Market_Sell" in LOCATIONS and LOCATIONS["Back_Rect_Market_Sell"].collidepoint(pos):
            # Không reset market_slot_end_time (vẫn còn thuê chỗ)
            game_state = "MARKET_MENU"
            return

    # --- [MỚI: SỰ KIỆN GIÁNG SINH] - Xử lý click màn hình quà Giáng Sinh ---
    ...
    # --- Xử lý click các màn hình khác (Giữ nguyên) ---
        ...
        if "Shop_Upgrade_Rod_Limit_Rect" in LOCATIONS and LOCATIONS["Shop_Upgrade_Rod_Limit_Rect"] is not None and LOCATIONS["Shop_Upgrade_Rod_Limit_Rect"].collidepoint(pos):
            # --- [SỬA: CHI PHÍ NÂNG CẤP CẤP SỐ NHÂN] ---
            upgrade_rod_limit_cost = 50000 * (2 ** (MAX_RODS - 5))  # 50k, 100k, 200k, 400k, 800k
            if player_coins >= upgrade_rod_limit_cost and MAX_RODS < 6:  # Tối đa 6 cần câu
                ...
                if "Rect" in slot and slot["Rect"].collidepoint(pos):
                    ...
                    if slot["catch_data"]:
                        ...
                        fish_inventory.append({
                            ...
                            "exp": catch['exp'],
                            "id": generate_fish_id() # Gán ID khi trộm
                        })
